...
const FIXTURE_FILE = 'allrecipes-vegan-shepherds-pie.html';
const EXPECTED_METHOD = 'DataAttributes'; // Expected: WPRM | DataAttributes | JSON-LD | HTML
...
async function runTest() {
  ...
  const recipe = await scrapeRecipeFromHTML(html, `https://www.allrecipes.com/recipe/180735/traditional-style-vegan-shepherds-pie/`);
  ...
  // Check for sections
  const sections = [...new Set(recipe.ingredients.map(i => i.section).filter(Boolean))];
  ...
  const validations = [
    ...
    { check: sections.includes('Mashed potato layer:'), message: 'Has "Mashed potato layer:" section' },
    { check: sections.includes('Bottom layer:'), message: 'Has "Bottom layer:" section' },
  ];
